...
class _af_prep:
    ...
    def _prep_offtarget_model(self, **kwargs):
        """
        Prepare an off-target model for each entry in self._offtargets.
        Previously, this code only handled one off-target by referencing
        `self._lengths_offtarget`. Now we have multiple off-targets,
        each with its own lengths, inputs, etc. stored in self._offtargets[i].
        """
        if not hasattr(self, "_offtargets") or len(self._offtargets) == 0:
            # no off-targets defined, do nothing
            return

        # We'll store a separate model for each off-target in a list
        # (or, you can store them back into each `offdict["_offtarget_model"]`).
        self._offtarget_models = []

        for i, offdict in enumerate(self._offtargets):
            
            # The old code (for just one off-target) was:
            #
            #   if not hasattr(self, "_offtarget_model") or self._cfg != self._offtarget_model["runner"].config:
            #       self._cfg.model.global_config.subbatch_size = None
            #       self._offtarget_model = self._get_model_offtarget(self._cfg)
            #       if sum(self._lengths_offtarget) > 384:
            #           self._cfg.model.global_config.subbatch_size = 4
            #           self._offtarget_model["fn"] = self._get_model_offtarget(self._cfg)["fn"]
            #   self._opt_offtarget = copy_dict(self.opt_offtarget)
            #   self.restart_offtarget(**kwargs)
            #
            # Now we adapt it to each off-target in self._offtargets:

            # (1) Possibly reset subbatch_size
            self._cfg.model.global_config.subbatch_size = None

            # each off-target's lengths are in offdict["lengths_offtarget"]
            off_len_sum = sum(offdict["lengths_offtarget"])
            if off_len_sum > 384:
                self._cfg.model.global_config.subbatch_size = 4

            # (2) build the model
            # typically _get_model_offtarget(...) returns something like
            # { "grad_fn":..., "fn":..., "runner":... }
            local_model = self._get_model_offtarget(self._cfg)

            # if we changed subbatch_size after building, we might do it again:
            if off_len_sum > 384:
                # re-build or reassign fn?
                local_model["fn"] = self._get_model_offtarget(self._cfg)["fn"]

            # store the result
            offdict["_offtarget_model"] = local_model

            # (3) copy or set local opt
            offdict["_opt_offtarget"] = copy_dict(self.opt_offtarget)

            # (4) If you want to call self.restart_offtarget(...) once per off-target,
            # you can do that here. e.g.
            self.restart_offtarget(**kwargs)

            # store in a parallel list, if desired
            self._offtarget_models.append(local_model)

        # Done. Now each off-target dict has its own model in offdict["_offtarget_model"].
        # Or we collected them in self._offtarget_models.
    ...
    def _prep_fixbb(self, pdb_filename, chain=None,
        ...
        if copies > 1:
            ...
            self._args.update({"copies": copies, "repeat": repeat,
                              "homooligomer": homooligomer, "block_diag": block_diag})
            homooligomer = not repeat
        else:
            self._lengths = self._pdb["lengths"]

        # configure input features
        self._inputs = self._prep_features(
            num_res=sum(self._lengths), num_seq=num_seq)
        self._inputs["residue_index"] = res_idx
        self._inputs["batch"] = make_fixed_size(
            self._pdb["batch"], num_res=sum(self._lengths))
        self._inputs.update(get_multi_id(
            self._lengths, homooligomer=homooligomer))
    ...
    def _prep_binder(self, pdb_filename,
        ...
        if redesign:
            self._target_len = sum(
                [(self._pdb["idx"]["chain"] == c).sum() for c in target_chain.split(",")])
            self._binder_len = sum(
                [(self._pdb["idx"]["chain"] == c).sum() for c in binder_chain.split(",")])
        else:
            self._target_len = self._pdb["residue_index"].shape[0]
            self._binder_len = binder_len
            res_idx = np.append(
                res_idx, res_idx[-1] + np.arange(binder_len) + 50)
        ...
        # configure input features
        self._inputs = self._prep_features(
            num_res=sum(self._lengths), num_seq=1)
        self._inputs["residue_index"] = res_idx
    ...
    def _prep_partial(self, pdb_filename, chain=None, length=None,
        ...
        if pos is None:
            ...
            self._pos_info = {"length": np.array(
                [self._pdb["len"]]), "pos": self._pdb["pos"]}
        else:
        ...
        if copies > 1:
            ...
            if repeat or homooligomer:
                ...
                self.opt["pos"] = self._pdb["pos"][self._pdb["pos"]
                                                   < self._pdb["len"]]

                # repeat positions across copies
                self._pdb["pos"] = repeat_pos(
                    self.opt["pos"], copies, self._pdb["len"])
    ...
    def _prep_binder_offtargets(self, off_list, **kwargs):
        """
        Accept a list of dictionaries, each specifying an off-target’s info:
          {
            "pdb_filename": "off1.pdb",
            "offtarget_chain": "A",
            "binder_len": 50,
            "binder_chain": None,
            "rm_offtarget": False,
        ...
        We'll call _prep_single_offtarget(...) for each, store the results in
        self._offtargets as a list of dicts.
        """
        if not isinstance(off_list, list):
            off_list = [off_list]  # handle single item

        self._offtargets.clear()

        for item in off_list:
            # parse a single off-target
            data_i = self._prep_single_offtarget(**item, **kwargs)
            self._offtargets.append(data_i)

        self._prep_offtarget_model(**kwargs)
        # done collecting off-target data
        # (Optionally build anything else needed here; e.g. if you want
        #  a single aggregator or something. We'll do it in `_get_model_offtargets`.)
    
    def _prep_single_offtarget(self,
                            pdb_filename,
                            offtarget_chain="A",
                            binder_len=50,
                            binder_chain=None,
                            rm_offtarget=False,
                            rm_offtarget_seq=False,
                            rm_offtarget_sc=False,
                            rm_binder=True,
                            rm_binder_seq=True,
                            rm_binder_sc=True,
                            rm_template_ic=False,
                            ...
                            **kwargs):
        """
        Prepares the features for *one* off-target. 
        Similar to _prep_binder, but returning a dict of data for that off-target.
        """
        # Are we redesigning an existing binder chain?
        
        redesign = (binder_chain is not None)
        rm_binder = not kwargs.pop("use_binder_template", not rm_binder)

        # Parse PDB
        chains = f"{offtarget_chain},{binder_chain}" if redesign else offtarget_chain
        im = [True] * len(offtarget_chain.split(","))
        if redesign:
            im += [ignore_missing] * len(binder_chain.split(","))
        pdb_data = prep_pdb(pdb_filename, chain=chains, ignore_missing=im)
        res_idx = pdb_data["residue_index"]

        # Figure out lengths (the crucial fix: cast to np.array before sum)
        if redesign:
            offtarget_len = sum(
                np.array(pdb_data["idx"]["chain"] == c).sum()
                for c in offtarget_chain.split(",")
            )
            binder_len_offtarget = sum(
                np.array(pdb_data["idx"]["chain"] == c).sum()
                for c in binder_chain.split(",")
            )
        else:
            offtarget_len = pdb_data["residue_index"].shape[0]
            binder_len_offtarget = binder_len
            # artificially extend residue indices for hallucinated region
            res_idx = np.append(res_idx, res_idx[-1] + np.arange(binder_len_offtarget) + 50)

        lengths_offtarget = [offtarget_len, binder_len_offtarget]

        # local copy of off-target options
        local_opt_off = copy_dict(self.opt_offtarget)

        # hotspot logic
        if hotspot is not None:
            local_opt_off["hotspot"] = prep_pos(hotspot, **pdb_data["idx"])["pos"]

        # If redesign vs. hallucination
        if redesign:
            wt_aatype_offtarget = pdb_data["batch"]["aatype"][offtarget_len:]
            local_opt_off["weights"].update({
                "dgram_cce": 1.0, "rmsd": 0.0, 
                "fape": 0.0, "con": 0.0, 
                "i_con": 0.0, "i_pae": 0.0
            })
        else:
            pdb_data["batch"] = make_fixed_size(
                pdb_data["batch"], num_res=(offtarget_len + binder_len_offtarget)
            )
            wt_aatype_offtarget = None
            local_opt_off["weights"].update({
                "plddt": 0.1, "con": 0.0, 
                "i_con": 1.0, "i_pae": 0.0
            })
        ...
        # Build features
        L_total = offtarget_len + binder_len_offtarget
        off_inputs = prep_input_features(L=L_total, N=1, T=1)
        off_inputs["residue_index"] = res_idx
        off_inputs["batch"] = pdb_data["batch"]
        off_inputs.update(get_multi_id([offtarget_len, binder_len_offtarget]))
        # off_inputs["opt"]["offtarget_len"] = offtarget_len

        # Template removal masks
        T = offtarget_len
        rm = {}
        rm_map = {
            "rm_template":     {"offtarget": rm_offtarget,    "binder": rm_binder},
            "rm_template_seq": {"offtarget": rm_offtarget_seq, "binder": rm_binder_seq},
            "rm_template_sc":  {"offtarget": rm_offtarget_sc,  "binder": rm_binder_sc}
        }
        for mask_name, reg_dict in rm_map.items():
            arr = np.full(L_total, False)
            for region_k, val in reg_dict.items():
                if isinstance(val, str):
                    arr[prep_pos(val, **pdb_data["idx"])["pos"]] = True
                else:
                    if region_k == "offtarget":
                        arr[:T] = val
                    if region_k == "binder":
                        arr[T:] = val
            rm[mask_name] = arr

        local_opt_off["template"].update({"rm_ic": rm_template_ic})
        off_inputs.update(rm)
        
        # Now offtarget_len is a genuine Python/NumPy int
        # so slice indexing works in JAX
        off_inputs["offtarget_len"] = offtarget_len
        off_inputs["binder_len_offtarget"] = binder_len_offtarget
        off_inputs["offtarget_len_static"] = int(offtarget_len)
        off_inputs["binder_len_offtarget_static"] = int(binder_len_offtarget)
        # Optionally copy over a bias if present
        if "bias" in self._inputs:
            off_inputs["bias"] = self._inputs["bias"]
        # Also store an index for this off-target
        self.offtarget_len = int(offtarget_len)
        # Return a dictionary describing this off-target
        off_inputs["opt"] = local_opt_off
        self.offtarget_len  = int(offtarget_len)
        ...
        return {
            ...
            "name": name,
            "role": role,
            # Store the true off-target length; binder length is kept separately
            "offtarget_len": offtarget_len,
            "lengths_offtarget": lengths_offtarget,
            "inputs": off_inputs,
            "opt_offtarget": local_opt_off,
            "wt_aatype_offtarget": wt_aatype_offtarget
        }
...
def repeat_idx(idx, copies=1, offset=50):
    idx_offset = np.repeat(
        np.cumsum([0]+[idx[-1]+offset]*(copies-1)), len(idx))
...
def prep_pdb(pdb_filename, chain=None,
    ...
    for n, chain in enumerate(chains):
        ...
        im = ignore_missing[n] if isinstance(
            ignore_missing, list) else ignore_missing
        if im:
            ...
            # pad values
            offset = 0 if offsets is None else (
                offsets[n] if isinstance(offsets, list) else offsets)
            r = offset + (protein_obj.residue_index -
                          protein_obj.residue_index.min())
            length = (r.max()+1) if lengths is None else (
                lengths[n] if isinstance(lengths, list) else lengths)
    ...
    o["idx"] = {"residue": np.concatenate(
        residue_idx), "chain": np.concatenate(chain_idx)}
    ...
    for k, v in feat.items():
        if k == "batch":
            ...
            assert len(shape) == len(schema), (
                ...
                f'{shape} vs {schema}')
            pad_size = [pad_size_map.get(s2, None) or s1 for (
                s1, s2) in zip(shape, schema)]
